...
namespace WP_PluginSafetyValidator\Support\Templates;
...
class Template
{
    ...
    public static function register_styles_and_scripts(string $type, string $context = 'frontend'): void
    {
        ...
        foreach ( glob( $file_dir . '*.'. $type ) as $file_path ) { // loop through all files in the directory
            $handle = WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN . '-' . sanitize_title( basename( $file_path, '.'. $type ) );
    ...
    /**
     * Filterable namespace used in themes for overrides, e.g. <theme>/<WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN>/admin/file.php
     *
     * @return string
     */
    public static function get_template_namespace(): string
    {
        return apply_filters( WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN .'_template_namespace', WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN );
    }

    /**
     * Filterable plugin base dir (absolute). Defaults to current file's plugin dir.
     *
     * @return string
     */
    public static function get_base_dir(): string
    {
        return trailingslashit( apply_filters( WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN .'_template_base_dir', WP_PLUGIN_SAFETY_VALIDATOR_DIR ) );
    }
     ...
     * @return string
     */
    public static function normalize_template_name(string $template_name ): string
    {
        $template_name = trim( $template_name );
        $template_name = ltrim( $template_name, '/\\' );
        $template_name = str_replace( array( '..', "\0" ), '', $template_name );

        return preg_replace( '#\\+#', '/', $template_name );
    }

    /**
     * Locate a template with override support (child → parent → plugin).
     *
     * @param string $template_name Without .php (e.g., 'metabox' or 'partials/item').
     * @param string $context       'admin' or 'frontend'.
     * @return string               Absolute path or empty string.
     */
    public static function locate_template(string $template_name, string $context ): string
    {
        ...
        $theme_candidates = array(
            ...
            trailingslashit( get_stylesheet_directory() ) . "$context/templates/$filename",
            trailingslashit( get_template_directory() )   . "$context/templates/$filename",
        );

        /**
         * Allow other code to adjust theme candidate paths.
         *
         * @param array  $theme_candidates
         * @param string $template_name
         * @param string $context
         */
        $theme_candidates = apply_filters( WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN .'_theme_template_candidates', $theme_candidates, $template_name, $context );

        foreach ( $theme_candidates as $path ) {
            if ( $path && is_readable( $path ) ) {
                /** @param string $path */
                return apply_filters( WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN .'_located_template', $path, $template_name, $context );
            }
        }

        // Plugin fallbacks: /admin/templates or /frontend/templates.
        $plugin_candidates = array(
            self::get_base_dir() . "$context/templates/$filename",
        );

        /**
         * Allow other code to add/modify plugin candidates.
         *
         * @param array  $plugin_candidates
         * @param string $template_name
         * @param string $context
         */
        $plugin_candidates = apply_filters( WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN .'_plugin_template_candidates', $plugin_candidates, $template_name, $context );

        foreach ( $plugin_candidates as $path ) {
            if ( $path && is_readable( $path ) ) {
                return apply_filters( WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN .'_located_template', $path, $template_name, $context );
            }
        }
     ...
     *
     * @param string $template_name Relative name without .php.
     * @param array $args          Variables exposed to the template.
     * @param bool $echo          Echo or return.
     * @param string $context       'admin' or 'frontend'.
     * @return string               Rendered HTML.
     */
    public static function render_template(string $template_name, array $args = [], bool $echo = true, string $context = 'frontend' ): string
    {
        $path = self::locate_template( $template_name, $context );
         ...
         * @param string $context
         */
        $path = apply_filters( WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN .'_before_render_template', $path, $template_name, $args, $context );
         ...
         * @param string $context
         */
        $html = apply_filters( WP_PLUGIN_SAFETY_VALIDATOR_DOMAIN .'_rendered_template_html', $html, $template_name, $args, $context );
     ...
     * @return string
     */
    public static function load_admin_view(string $template_name, array $args = [], bool $echo = false ): string
    {
        return self::render_template( $template_name, $args, $echo, 'admin' );
    }
     ...
     * @return string
     */
    public static function load_frontend_view(string $template_name, array $args = [], bool $echo = false ): string
    {
        return self::render_template( $template_name, $args, $echo );
    }
}